...
<{block "title"}>分类设置<{/block}>

<{block "name"}>web/category<{/block}>
<{block "head-plus"}>
...
<link rel="stylesheet" href="<{'css/catalog/catalog.css'|plugins nofilter}>">
    <script src="<{'js/category/category.min.js'|static nofilter}>"></script>
    <{/if}>
    <script>
        (function($){
            $().ready(function(){
    ...
    <div class="block full">
            ...
            <nav class="navbar navbar-default">
                ...
                <div class="collapse navbar-collapse">
                    <ul class="nav navbar-nav">
                        ...
                        <li class="<{if $_root->getKey() == $node->getKey()}>active<{/if}>"><a href="<{if !empty
                            ($_urlPrefix)}><{$_urlPrefix nofilter}><{else}><{'admin/web/category'|url}><{/if}>/<{$node->getKey()}>"><{$node->title}>
                                (<{$node->name}>)</a></li>
                        <{/foreach}>
        ...
        <div class="block-content">
            ...
            <div class="well well-lg col-sm-10 col-sm-offset-1">
                ...
                <ul>
                    <{foreach $_topNodes as $node}>
                    <li><a href="<{if !empty($_urlPrefix)}><{$_urlPrefix nofilter}><{else}><{'admin/web/category'|url}><{/if}>/<{$node->getKey()}>"><{$node->title}>
                            (<{$node->name}>)</a></li>
                    <{/foreach}>
                </ul>
            </div>
            ...
            <div class="row catalog-container">
                ...
                <div class="col-md-8">
                    <div class="catalog-form" id="catalog-form" url-prefix="<{if !empty($_urlPrefix)}><{$_urlPrefix nofilter}><{else}><{'admin/web/category'|url}><{/if}>">
